...
namespace BLL.InHostory
{
    public class InHostory
    {
        ...
        {
            string SelectPid = @"select ROW_NUMBER() OVER(ORDER BY o.Pid)as xuhao,o.Pid, Mname,DataText,Mnumber,UnitName,PropertySum ,SCompanyName,PropertyToal,PropertyNote from Orderhistory o left join PurchaseProperty p on o.Pid = p.Pid left join dbo.Merchandise m on p.mid = m.MId
